...
const AddMenu = () => {
  ...
  const { createMenu, loading } = useMenuStore(); // Get menus from the store
  ...
  const submitHandler = async (e: FormEvent<HTMLFormElement>) => {
    ...
    // Call createMenu and reset state
    try {
  ...
  return (
    <div className="max-w-6xl mx-auto my-10">
          ...
          <DialogContent>
            ...
            <form onSubmit={submitHandler} className="space-y-4">
              ...
              <div>
                ...
                />
                {error.image && <span className="text-xs font-medium text-red-600">{error.image?.name}</span>}
              </div>
      ...
      {/* Render menus from the store */}
      {restaurant?.menus.map((menu:any, idx:number) => (
        <div key={idx} className="mt-6 space-y-4">
          <div className="flex flex-col md:flex-row md:items-center md:space-x-4 md:p-4 p-2 shadow-md border">
